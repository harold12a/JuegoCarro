...
public class Juego {
    ...
    public Juego(Integer numCarriles,Integer kilometros, String nombrePista) {
        Map<Integer,Carril> carriles = generarCarriles(numCarriles,kilometros);
        pista = new Pista(UUID.randomUUID().toString(),carriles,nombrePista);
    ...
    private Map<Integer,Carril> generarCarriles(Integer numCarriles, Integer kilometros){
     ...
     private void validarCarriles(){
        ...
        if(cantidadDeCarros<=2){
            throw new IllegalStateException("no tiene la cnatidad de carros suficientes");
        }
